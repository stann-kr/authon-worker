...
- 데모의 실제 화면 모드는 desktop 좌측 업무 탐색, mobile 수평 업무 탐색과 상단 영업 상태 strip을 사용한다. 가이드 모드와 같은 로컬 상태를 공유하되 작업별 독립 화면과 현재 화면을 나타내는 query를 제공한다.
- 영업 상태 strip은 장식용 지표가 아니라 도어 운영 여부, 입장 대기·완료 인원과 현재 역할에서 조회 가능한 미처리 업무만 보여준다. 권한이 없는 역할에는 승인 건수를 노출하지 않는다.
