...
- 포트폴리오 데모는 핵심 운영 흐름을 네 단계 scenario rail로 보여준다. 각 단계는 실제 작업 화면으로 이동하며, 완료 상태는 아이콘·텍스트·상태색을 함께 사용하고 순서와 무관하게 진행할 수 있다.
- 데모 로그인은 일반적인 상품 소개 hero 대신 오늘 밤 근무자 명단을 역할 선택 장치로 사용한다. 계정 카드는 자격 증명을 채우는 보조 수단이며 실제 로그인 form과 실패 상태를 유지한다.
- 데모의 실제 화면 모드는 운영 서비스와 같은 고정 App Header, 역할별 Workspace Menu, Guest·Door의 master-detail 운영 레이아웃과 Admin tab 구조를 사용한다. 가이드 모드와 같은 로컬 상태를 공유하되 현재 업무를 나타내는 query를 제공한다.
- 데모 전용 계정 전환·초기화·가이드 복귀는 App Header의 보조 action으로 제한하고, 핵심 업무 정보 구조와 입력 필드는 운영 서비스와 다르게 재설계하지 않는다.
