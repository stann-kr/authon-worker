...
| `/demo` | 공개 | 가상 계정 로그인과 역할별 접근을 포함한 브라우저 로컬 포트폴리오 데모 |
| `/demo/workspace` | 공개 | 운영 앱과 같은 Home·Guest·Door·Admin UI에서 역할별 업무를 체험하는 브라우저 로컬 데모 |
...
- `/demo`는 공개된 가상 `venue_admin`, `door_staff`, `dj` 계정으로 로그인해 게스트 등록, 도어 체크인, 추가 한도 승인, 외부 게스트 링크 생성 흐름을 순서형 가이드로 체험하는 샌드박스다.
- `/demo/workspace`는 같은 가상 세션과 상태를 사용하면서 운영 앱과 공유하는 App Header, Workspace Menu, 운영 layout과 공통 component로 Home·Guest·Door·Admin 업무를 제공한다.
- 가상 로그인 세션은 `sessionStorage`, 샘플 상태와 사용자의 변경은 `localStorage`에 저장하며 인증 cookie, API route, D1, KV를 사용하지 않는다.
