...
| `/demo/workspace` | 공개 | 실제 앱 셸에서 역할별 운영 업무를 체험하는 브라우저 로컬 데모 |
...
- `/demo/workspace`는 같은 가상 세션과 상태를 사용하면서 좌측·모바일 업무 탐색, 영업 상태 strip과 독립 작업 영역을 갖춘 실제 앱 구성으로 같은 업무를 제공한다.
