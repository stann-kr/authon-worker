...
| Account Setup | 이관 계정의 최초 1회 직접 설정과 관리자 발급 1회용 설정 코드 기반 비밀번호 설정 |
...
| `/api/auth/claim-account` | 공개 API | 이관 대기 계정 또는 검증된 1회용 설정 코드로 최초 비밀번호 설정 |
...
- `migrated_at`이 기록된 계정은 `pending_reset`이면서 비밀번호를 아직 설정하지 않은 경우 임의의 최초 로그인 입력으로 설정 단계에 진입한다. 이후 관리자 재설정 요청이 있으면 설정 코드를 요구하고, 잘못 시작한 요청을 취소한 최신 감사 기록이 있으면 이관 직접 설정으로 복원한다.
- 관리자 생성·재설정 계정은 같은 대기 조건과 함께 별도로 전달한 1회용 설정 코드 hash까지 확인한다.
- 설정 성공 시 계정 상태와 session version을 원자적으로 변경하고 기존 reset token을 모두 사용 처리한다.
- 설정 요청은 IP와 이메일 조합으로 rate limit하며, 완료된 이관 설정 경로와 설정 코드는 재사용할 수 없다.
