...
- 공개 요청은 24시간짜리 HttpOnly 서명 영수증과 별도 요청 확인 문구를 발급한다. 관리자는 대면, 등록 전화 또는 기존에 확인된 메신저로 사용자를 먼저 확인한 뒤 사용자가 알려준 문구를 정확한 요청에 입력한다.
- 코드 없는 승인은 개인 `door_staff`, `staff`, `dj`의 self-service 요청에만 허용하며, 같은 브라우저의 영수증으로 15분 안에 한 번만 소비할 수 있다. 사용자 목록의 수동 재설정과 관리자·공용 계정은 설정 코드만 사용한다.
- `pending_reset`이면서 비밀번호를 아직 설정하지 않은 계정의 로그인 설정 단계는 현재 password hash와 일치하는 초기 설정 코드로만 시작한다. 관리자가 새로 발급한 코드는 미사용·미만료 승인 요청에도 결속하며, 코드 없는 승인은 별도의 요청 화면에서만 소비한다.
