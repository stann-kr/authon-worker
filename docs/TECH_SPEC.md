...
| Account Setup | `pending_reset` 계정의 1회용 설정 코드 또는 만료되는 관리자 승인 기반 비밀번호 설정 |
...
| `/api/auth/claim-account` | 공개 API | 검증된 1회용 설정 코드 또는 유효한 관리자 승인으로 `pending_reset` 계정의 비밀번호 설정 |
...
- 공개 관리자 요청은 계정 존재 여부, tenant 불일치, 기존 열린 요청 여부에 같은 성공 응답을 사용하고 IP와 이메일 조합으로 rate limit한다.
- `pending_reset`이면서 비밀번호를 아직 설정하지 않은 계정은 현재 password hash와 일치하는 관리자 발급 1회용 설정 코드 또는 전용 요청 테이블의 미사용·미만료 관리자 승인이 있을 때만 설정 단계에 진입한다.
- 코드 없는 관리자 승인은 24시간 동안 한 번만 유효하며, 감사 기록은 인증 허용 여부의 근거로 사용하지 않는다.
- 설정 성공 시 계정 상태와 session version, 열린 요청 상태를 원자적으로 변경하고 기존 reset token을 모두 사용 처리한다. 사용 완료된 설정 코드와 승인은 재사용할 수 없다.
- 관리자 결정은 role, venue scope, 자기 계정, 비활성·삭제 상태를 서버에서 다시 검증하고 기존 세션과 reset token을 무효화한다.
