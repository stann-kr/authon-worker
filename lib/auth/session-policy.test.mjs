...
test("remembered sessions extend to 30 days from every later valid activity", () => {
  const lifetime = createLoginSessionLifetime(true, NOW);
  const session = createStoredSession("user-1", 3, lifetime);
  const lastUse = new Date(
    (NOW_SECONDS + 10 * 24 * 60 * 60) * 1000,
  );

  assert.deepEqual(
    getRememberedSessionRefresh(session, lifetime.expiresAtSeconds, lastUse),
    {
      expiresAtSeconds: Math.floor(lastUse.getTime() / 1000) + REMEMBERED_SESSION_IDLE_TTL_SECONDS,
