...
const profileSql = {
  updatePassword: UPDATE_PROFILE_PASSWORD_CAS_SQL,
  insertAudit: INSERT_PROFILE_PASSWORD_AUDIT_SQL,
  invalidateTokens: INVALIDATE_PROFILE_RESET_TOKENS_SQL,
  cancelRequests: CANCEL_PROFILE_RESET_REQUESTS_SQL,
};

const tokenSql = {
  selectCandidate: SELECT_VALID_RESET_TOKEN_CANDIDATE_SQL,
  updatePassword: UPDATE_PASSWORD_WITH_VALID_TOKEN_SQL,
  consumeExactToken: CONSUME_EXACT_RESET_TOKEN_SQL,
  insertAudit: INSERT_TOKEN_RESET_AUDIT_SQL,
  invalidateAllTokens: INVALIDATE_ALL_USER_RESET_TOKENS_SQL,
  completeRequests: COMPLETE_TOKEN_RESET_REQUESTS_SQL,
};
...
const loginSql = {
  selectLatestSetupCodeRequest: SELECT_LATEST_SETUP_CODE_REQUEST_SQL,
  updateUser: UPDATE_USER_FOR_LOGIN_SQL,
  cancelRequests: CANCEL_OPEN_PASSWORD_RESET_REQUESTS_AFTER_LOGIN_SQL,
};
...
test("비활성 venue는 profile password의 최종 CAS 쓰기도 거부한다", () => {
  const db = createDatabase();
  seedOpenCredentialState(db);
  db.exec("UPDATE venues SET active = 0 WHERE id = 'venue-a'");

  const [passwordResult, auditResult] = runProfileChange(db, {
    auditId: "inactive-profile-audit",
    expectedHash: "old-hash",
    expectedSessionVersion: 7,
    nextHash: "blocked-profile-hash",
  });
...
test("super admin은 venue 없이도 profile password recovery CAS를 수행할 수 있다", () => {
  const db = createDatabase();
  seedOpenCredentialState(db);
  db.exec(`
    UPDATE venues SET active = 0 WHERE id = 'venue-a';
    UPDATE users SET role = 'super_admin', venue_id = NULL WHERE id = 'user-a';
  `);

  const [passwordResult, auditResult] = runProfileChange(db, {
    auditId: "super-admin-profile-audit",
    expectedHash: "old-hash",
    expectedSessionVersion: 7,
    nextHash: "recovered-profile-hash",
  });
  ...
  assert.deepEqual(
    ...
    { password_hash: "recovered-profile-hash", session_version: 8 },
  );
});
