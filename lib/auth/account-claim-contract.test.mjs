...
const [route, handler, service, persistence] = await Promise.all([
  ...
  readFile(new URL("./account-claim-route-handler.ts", import.meta.url), "utf8"),
...
test("account claim production route wires only capability adapters into its executable handler", () => {
  assert.match(route, /createAccountClaimPostHandler/);
...
test("account claim handler owns request, origin, rate-limit, response, cookie, and telemetry policy", () => {
  assert.match(handler, /isTrustedMutationOrigin\(request\)/);
  assert.match(handler, /await request\.json\(\)\.catch/);
  assert.match(handler, /dependencies\.consumeRateLimit/);
  assert.match(handler, /dependencies\.getTenant\(request\)/);
  assert.match(handler, /dependencies\.claim\(/);
  assert.match(handler, /PASSWORD_RESET_CLAIM_COOKIE_NAME/);
  assert.match(handler, /PASSWORD_RESET_RECEIPT_COOKIE_NAME/);
  assert.match(handler, /signedReceiptRequestId === result\.requestId/);
  assert.match(handler, /writeLog\("info"/);
  assert.match(handler, /reportError\("auth\.account_claim"/);
  assert.doesNotMatch(
    handler,
    /env\.DB\.prepare|env\.DB\.batch|SELECT u\.id|UPDATE_USER_WITH_APPROVED_RESET_SQL/,
  );
});
