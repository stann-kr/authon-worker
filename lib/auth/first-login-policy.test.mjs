import assert from "node:assert/strict";
import test from "node:test";

import {
  canStartFirstLoginSetup,
  getFirstLoginSetupMethod,
  isFirstLoginResetControlAction,
  isFirstLoginSetupMethod,
} from "./first-login-policy.ts";

test("이관 대기 계정은 임의의 최초 로그인 입력으로 설정을 시작한다", () => {
  const method = getFirstLoginSetupMethod({
    latestResetControlAction: null,
    migrationStatus: "pending_reset",
    migratedAt: "2026-08-07T00:00:00.000Z",
    passwordSetAt: null,
  });

  assert.equal(method, "migration");
  assert.equal(canStartFirstLoginSetup(method, false), true);
});

test("관리자 생성·재설정 계정은 올바른 설정 코드가 있어야 한다", () => {
  const method = getFirstLoginSetupMethod({
    latestResetControlAction: null,
    migrationStatus: "pending_reset",
    migratedAt: null,
    passwordSetAt: null,
  });

  assert.equal(method, "setup_code");
  assert.equal(canStartFirstLoginSetup(method, false), false);
  assert.equal(canStartFirstLoginSetup(method, true), true);
});

test("이관 이력이 있어도 관리자 재설정 뒤에는 설정 코드를 요구한다", () => {
  const method = getFirstLoginSetupMethod({
    latestResetControlAction: "password_reset_required",
    migrationStatus: "pending_reset",
    migratedAt: "2026-08-07T00:00:00.000Z",
    passwordSetAt: null,
  });

  assert.equal(method, "setup_code");
  assert.equal(canStartFirstLoginSetup(method, false), false);
  assert.equal(canStartFirstLoginSetup(method, true), true);
});

test("관리자 재설정 취소가 최신 기록이면 이관 직접 설정을 복원한다", () => {
  const method = getFirstLoginSetupMethod({
    latestResetControlAction: "password_reset_cancelled",
    migrationStatus: "pending_reset",
    migratedAt: "2026-08-07T00:00:00.000Z",
    passwordSetAt: null,
  });

  assert.equal(method, "migration");
  assert.equal(canStartFirstLoginSetup(method, false), true);
});

test("취소 뒤 다시 관리자 재설정을 요청하면 설정 코드를 다시 요구한다", () => {
  const method = getFirstLoginSetupMethod({
    latestResetControlAction: "password_reset_required",
    migrationStatus: "pending_reset",
    migratedAt: "2026-08-07T00:00:00.000Z",
    passwordSetAt: null,
  });

  assert.equal(method, "setup_code");
  assert.equal(canStartFirstLoginSetup(method, false), false);
});

test("비이관 계정은 재설정 취소 기록만으로 직접 설정 대상이 되지 않는다", () => {
  const method = getFirstLoginSetupMethod({
    latestResetControlAction: "password_reset_cancelled",
    migrationStatus: "pending_reset",
    migratedAt: null,
    passwordSetAt: null,
  });

  assert.equal(method, "setup_code");
  assert.equal(canStartFirstLoginSetup(method, false), false);
});

test("설정이 끝났거나 대기 상태가 아닌 계정은 최초 로그인 대상이 아니다", () => {
  assert.equal(
    getFirstLoginSetupMethod({
      latestResetControlAction: null,
      migrationStatus: "active",
      migratedAt: "2026-08-07T00:00:00.000Z",
      passwordSetAt: null,
    }),
    null,
  );
  assert.equal(
    getFirstLoginSetupMethod({
      latestResetControlAction: null,
      migrationStatus: "pending_reset",
      migratedAt: "2026-08-07T00:00:00.000Z",
      passwordSetAt: "2026-08-07T01:00:00.000Z",
    }),
    null,
  );
});

test("클라이언트에는 알려진 최초 로그인 방식만 전달한다", () => {
  assert.equal(isFirstLoginSetupMethod("migration"), true);
  assert.equal(isFirstLoginSetupMethod("setup_code"), true);
  assert.equal(isFirstLoginSetupMethod("unknown"), false);
  assert.equal(isFirstLoginSetupMethod(null), false);
  assert.equal(isFirstLoginResetControlAction("password_reset_required"), true);
  assert.equal(isFirstLoginResetControlAction("password_reset_cancelled"), true);
  assert.equal(isFirstLoginResetControlAction("password_reset_completed"), false);
});
