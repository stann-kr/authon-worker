...
import {
  ...
  readPasswordResetClaimCookie,
  readPasswordResetReceiptCookie,
  verifyPasswordResetClaimGrant,
  verifyPasswordResetClaimGrantRecord,
...
test("요청 확인번호는 별도 HMAC purpose로 안정적인 4자리 숫자를 만든다", async () => {
  ...
  assert.equal(
    await verifyPasswordResetChallenge(REQUEST_ID, "12345", SECRET),
    false,
  );
});

test("승인 claim grant는 exact request와 만료 시각을 서명하고 15분 뒤 거부한다", async () => {
  const expiresAtMs = NOW_MS + PASSWORD_RESET_CLAIM_MAX_AGE_SECONDS * 1000;
  const grant = await createPasswordResetClaimGrant(
    REQUEST_ID,
    expiresAtMs,
    SECRET,
  );
  ...
  assert.match(
    grant,
    new RegExp(`^${REQUEST_ID}\\.\\d{10,13}\\.[A-Za-z0-9_-]{43}$`),
  );
  assert.deepEqual(
    await verifyPasswordResetClaimGrant(grant, SECRET, NOW_MS),
    {
  ...
  assert.equal(
    await verifyPasswordResetClaimGrant(grant, SECRET, expiresAtMs),
    null,
  );
  assert.deepEqual(
    await verifyPasswordResetClaimGrantRecord(grant, SECRET),
    {
      requestId: REQUEST_ID,
      expiresAt: new Date(expiresAtMs).toISOString(),
    },
  ...
  assert.equal(
    await verifyPasswordResetClaimGrant(grant, `${SECRET}-other`, NOW_MS),
    null,
  );
  const tampered = grant.replace(REQUEST_ID, OTHER_REQUEST_ID);
  assert.equal(
    await verifyPasswordResetClaimGrant(tampered, SECRET, NOW_MS),
    null,
  );
});
...
test("receipt cookie는 host 정책에 맞춘 Secure와 엄격한 browser 경계를 사용한다", () => {
  ...
  assert.deepEqual(getPasswordResetClaimCookieOptions(true), {
    httpOnly: true,
    secure: true,
    sameSite: "strict",
    maxAge: 15 * 60,
...
test("claim cookie에서 미만료 exact request만 복구한다", async () => {
  ...
  const headers = new Headers({
    cookie: `${PASSWORD_RESET_CLAIM_COOKIE_NAME}=${grant}`,
  });

  assert.equal(readPasswordResetClaimCookie(headers), grant);
  assert.deepEqual(
    await getPasswordResetClaimGrant(headers, SECRET, NOW_MS),
    {
      requestId: REQUEST_ID,
      expiresAt: new Date(expiresAtMs).toISOString(),
    },
  ...
  assert.equal(
    await getPasswordResetClaimGrant(headers, SECRET, expiresAtMs),
    null,
  );
  assert.deepEqual(
    await getPasswordResetClaimGrantRecord(headers, SECRET),
    {
      requestId: REQUEST_ID,
      expiresAt: new Date(expiresAtMs).toISOString(),
    },
