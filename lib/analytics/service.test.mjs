...
const walkInDayRows = [
  { businessDate: "2026-07-09", eventId: null, walkInCount: 4 },
  { businessDate: "2026-08-07", eventId: null, walkInCount: 6 },
  { businessDate: "2026-08-11", eventId: null, walkInCount: 3 },
];

const attendanceGuestScopeRows = guestDayRows.map((row) => ({
  businessDate: row.businessDate,
  eventId: null,
  checkedInCount: row.checkedInCount,
}));

const attendanceCloseoutRows = [];
...
test("attendance snapshots override only their exact scope and remain after guest rows disappear", () => {
  const view = buildAdminAnalyticsView({
    selection,
    eventRows,
    guestDayRows,
    attendanceGuestScopeRows: [
      { businessDate: "2026-07-09", eventId: null, checkedInCount: 10 },
      { businessDate: "2026-08-07", eventId: null, checkedInCount: 2 },
      { businessDate: "2026-08-07", eventId: "named-a", checkedInCount: 4 },
      { businessDate: "2026-08-07", eventId: "named-b", checkedInCount: 3 },
    ],
    walkInDayRows: [
      { businessDate: "2026-07-09", eventId: null, walkInCount: 4 },
      { businessDate: "2026-08-07", eventId: null, walkInCount: 1 },
      { businessDate: "2026-08-07", eventId: "named-a", walkInCount: 5 },
      { businessDate: "2026-08-07", eventId: "named-b", walkInCount: 2 },
    ],
    attendanceCloseoutRows: [
      {
        businessDate: "2026-07-09",
      ...
      },
      {
        ...
        checkedInGuests: 7,
        finalWalkIns: 8,
        targetTotalAttendance: 15,
      },
      {
        businessDate: "2026-08-08",
        eventId: "deleted-finalized-event",
        checkedInGuests: 3,
        finalWalkIns: 6,
        targetTotalAttendance: 9,
      },
    ],
    contributorRows,
  });

  assert.equal(view.attendance.summary.totalAttendance.value, 32);
  assert.equal(view.attendance.summary.totalAttendance.comparisonValue, 10);
  assert.equal(view.attendance.summary.checkedInGuests.value, 15);
  assert.equal(view.attendance.summary.walkIns.value, 17);
  assert.deepEqual(view.attendance.trend, [
    {
      bucketStartDate: "2026-08-07",
      checkedInGuests: 12,
...
test("invalid rows fail closed instead of returning partial analytics", () => {
  ...
  assert.throws(() =>
    buildAdminAnalyticsView({
      ...
      attendanceGuestScopeRows,
      walkInDayRows: [
        { businessDate: "2026-08-11", eventId: null, walkInCount: -1 },
      ],
      attendanceCloseoutRows,
      contributorRows,
    }),
  ...
  assert.throws(() =>
    buildAdminAnalyticsView({
      ...
      guestDayRows,
      attendanceGuestScopeRows: [
        { businessDate: "2026-08-07", eventId: null, checkedInCount: 1 },
        { businessDate: "2026-08-07", eventId: null, checkedInCount: 2 },
      ],
      walkInDayRows,
      attendanceCloseoutRows,
