...
test("attendance combines checked-in guests and walk-ins on actual entry days", () => {
  const aggregate = summarizeAnalyticsAttendanceDays([
    { businessDate: "2026-08-01", checkedInGuests: 10, walkIns: 4 },
    { businessDate: "2026-08-02", checkedInGuests: 0, walkIns: 6 },
  ]);
  assert.deepEqual(aggregate, {
    ...
    walkIns: 10,
    totalAttendance: 20,
    attendancePerOperatingDay: 10,
  });

  const comparison = summarizeAnalyticsAttendanceDays([
    { businessDate: "2026-07-01", checkedInGuests: 5, walkIns: 0 },
  ]);
  assert.deepEqual(buildAnalyticsAttendanceSummary(aggregate, comparison), {
    totalAttendance: compareAnalyticsMetric(20, 5, "number"),
    checkedInGuests: compareAnalyticsMetric(10, 5, "number"),
    walkIns: compareAnalyticsMetric(10, 0, "number"),
    attendancePerOperatingDay: compareAnalyticsMetric(10, 5, "number"),
  });
});

test("attendance rejects zero-only days, duplicate dates, and invalid counts", () => {
  assert.throws(() =>
    summarizeAnalyticsAttendanceDays([
      { businessDate: "2026-08-01", checkedInGuests: 0, walkIns: 0 },
    ]),
  );
  assert.throws(() =>
    summarizeAnalyticsAttendanceDays([
      { businessDate: "2026-08-01", checkedInGuests: 1, walkIns: 0 },
      { businessDate: "2026-08-01", checkedInGuests: 0, walkIns: 1 },
    ]),
  );
  assert.throws(() =>
    summarizeAnalyticsAttendanceDays([
      { businessDate: "2026-08-01", checkedInGuests: -1, walkIns: 1 },
    ]),
  );
});
