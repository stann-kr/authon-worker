...
test("analytics action aggregates walk-ins and closeouts by exact attendance scope", () => {
  assert.match(source, /from\(attendanceActivityLedger\)/);
  assert.match(source, /sum\(\$\{attendanceActivityLedger\.delta\}\)/);
  assert.match(
    source,
    /groupBy\(\s*attendanceActivityLedger\.businessDate,\s*attendanceActivityLedger\.eventId,/,
  );
  assert.match(source, /from\(attendanceCloseouts\)/);
  assert.match(source, /attendanceCloseouts\.targetTotalAttendance/);
  assert.match(source, /attendanceCloseouts\.finalWalkIns/);
...
test("analytics action keeps guest-day aggregates and separately normalizes attendance scopes", () => {
  assert.match(source, /Promise\.all/);
  assert.match(source, /MAX_ANALYTICS_QUERY_ROWS/);
  assert.match(source, /from\(guests\)/);
  assert.match(source, /groupBy\(guests\.date\)/);
  assert.match(source, /attendanceGuestScopeEventId/);
  assert.match(source, /eq\(events\.id, guests\.eventId\)/);
  assert.match(source, /eq\(events\.venueId, guests\.venueId\)/);
  assert.match(source, /eq\(events\.businessDate, guests\.date\)/);
  assert.match(source, /attendanceCloseoutRows\.length > MAX_ANALYTICS_QUERY_ROWS/);
