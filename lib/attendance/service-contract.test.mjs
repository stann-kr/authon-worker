...
test("manual reconciliation derives the delta in one compare-and-swap insert", () => {
  ...
  assert.match(persistence, /candidate\.source_activity_count = \?/);
  assert.match(source, /expectedSourceActivityCount/);
  assert.match(persistence, /"total_reconciliation"/);
  assert.equal(source.includes("delta: adjustment.delta"), false);
});

test("attendance closeout owns a frozen exact scope without changing guest closeout hashing", () => {
  assert.match(persistence, /attendance-closeout:v1/);
  assert.match(persistence, /attendance_closeouts/);
  assert.match(source, /isFinalized: true/);
  assert.match(source, /canFinalize:/);
  assert.match(source, /unavailableReason: "scope_closed"/);
  assert.match(persistence, /canApplyNew/);
});
