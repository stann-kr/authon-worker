...
test("latest pending walk-in remains undoable until a reversal is queued", () => {
  ...
  assert.equal(
    transitionOfflineAttendanceMutation(first, "scope_closed").state,
    "scope_closed",
  );
});
...
test("manual reconciliation derives a bounded delta from an absolute total", () => {
  assert.deepEqual(
    prepareAttendanceReconciliation({
      targetTotalAttendance: 12,
      expectedCheckedInGuests: 7,
      expectedWalkIns: 8,
      expectedSourceActivityCount: 19,
  ...
  assert.throws(
    () => prepareAttendanceReconciliation({
      ...
      expectedWalkIns: 8,
      expectedSourceActivityCount: 19,
      reason: "below checked guests",
    }),
    /INVALID_ATTENDANCE_RECONCILIATION/,
  );
  assert.throws(
    () => prepareAttendanceReconciliation({
      targetTotalAttendance: 516,
      expectedCheckedInGuests: 7,
      expectedWalkIns: 8,
      expectedSourceActivityCount: 19,
      reason: "too large",
