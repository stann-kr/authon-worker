...
test("a demo guest can be removed from the browser-local roster", () => {
  const seed = createDemoState();
  const next = deleteDemoGuest(seed, "guest-1");

  assert.equal(next.guests.some((guest) => guest.id === "guest-1"), false);
  assert.equal(seed.guests.some((guest) => guest.id === "guest-1"), true);
  assert.equal(deleteDemoGuest(next, "missing-guest"), next);
});
...
test("an approved quota request keeps the administrator's lower approved count", () => {
  const initial = createDemoState();
  const approved = decideDemoRequest(initial, "request-1", "approved", {
    createdAt: actionTime,
    approvedCount: 4,
  });

  assert.equal(approved.requests[0].approvedCount, 4);
});
