...
const quotaSql = await readFile(new URL("../guest-limits/mutation-sql.ts", import.meta.url), "utf8");
...
test("guest orchestration consumes Events and Guest Limits public contracts", () => {
  ...
  assert.match(quota, /INSERT_CURRENT_ACTOR_EVENT_CONTRIBUTOR_LIMIT_SQL/);
  assert.match(quotaSql, /event_contributor_limits/);
  assert.match(quotaSql, /ON CONFLICT\(event_id, user_id\) DO NOTHING/);
});
