...
test("authenticated soft delete enforces expected venue and ownership", () => {
  ...
  assert.equal(
    remove.get(NOW, "event-a", "guest", "venue-b", "event-a", 1, DATE, "event-a", 1, "user-b"),
    undefined,
  );
  assert.equal(
    remove.get(NOW, "event-a", "guest", "venue-a", "event-a", 1, DATE, "event-a", 0, "user-b"),
    undefined,
  );
  assert.equal(
    remove.get(NOW, "event-a", "guest", "venue-a", "event-a", 1, DATE, "event-a", 0, "user-a")?.id,
    "guest",
  );
  assert.equal(
    db.prepare("SELECT status FROM guests WHERE id = 'guest'").get().status,
...
test("restore is forward-only from deleted and requires an active writable event", () => {
  ...
  assert.equal(
    restore.get(NOW, "event-a", "guest", "venue-a", "event-a", 1, DATE, "event-a")?.id,
    "guest",
  );
  assert.equal(
    restore.get(NOW, "event-a", "guest", "venue-a", "event-a", 1, DATE, "event-a"),
    undefined,
  );
  assert.deepEqual(
