...
test("final persistence guard misses are never reported as successful mutations", async () => {
  const staleUpdate = createFakePersistence();
  staleUpdate.persistence.updateProfile = async (input) => {
    staleUpdate.calls.push(["updateProfile", input]);
    return false;
  };
  await assert.rejects(
    updateManagedUserProfile(
      ...
      {
        persistence: staleUpdate.persistence,
        requireActiveVenueId: async (venueId) => venueId,
      },
    ),
    assertUserError("UPDATE_FAILED"),
  );

  const lastAdmin = createFakePersistence(user({
    id: "super-target",
    venueId: null,
    role: "super_admin",
  }));
  let otherAdminChecks = 0;
  lastAdmin.persistence.hasAnotherActiveSuperAdmin = async () => {
    otherAdminChecks += 1;
    return otherAdminChecks === 1;
  };
  lastAdmin.persistence.updateProfile = async () => false;
  await assert.rejects(
    updateManagedUserProfile(
      {
        actor: actor({ id: "super-actor", role: "super_admin", venueId: null }),
      ...
      {
        persistence: lastAdmin.persistence,
        requireActiveVenueId: async (venueId) => venueId,
      },
    ),
    assertUserError("LAST_SUPER_ADMIN"),
  ...
  const blockedCreate = createFakePersistence();
  blockedCreate.persistence.createUser = async () => false;
  await assert.rejects(
    createManagedUser(
      {
        actor: actor(),
      ...
      {
        persistence: blockedCreate.persistence,
        requireActiveVenueId: async (venueId) => venueId,
        prepareAccountInvitation: async () => ({
          passwordHash: "inactive-password-hash",
          invitation: {
  ...
  const blockedDelete = createFakePersistence(user({ active: false }));
  blockedDelete.persistence.deleteUser = async () => false;
  await assert.rejects(
    deleteManagedUser(
      { actor: actor(), userId: "staff-a" },
      {
        persistence: blockedDelete.persistence,
        requireActiveVenueId: async (venueId) => venueId,
        createDeletedPasswordHash: async () => "deleted-hash",
      },
    ),
