...
test("user persistence keeps session invalidation adjacent to credential revocation and audit", () => {
  const sessionIndex = persistence.indexOf("dbUpdates.sessionVersion");
  const requestIndex = persistence.indexOf("passwordResetRequests", sessionIndex);
  const tokenIndex = persistence.indexOf("passwordResetTokens", requestIndex);
  const auditIndex = persistence.indexOf("auditStatement", tokenIndex);
  assert.ok(sessionIndex >= 0);
  assert.ok(requestIndex > sessionIndex);
  assert.ok(tokenIndex > requestIndex);
  assert.ok(auditIndex > tokenIndex);
  assert.match(persistence, /hasAnotherActiveSuperAdmin/);
  assert.match(persistence, /ne\(users\.id, targetId\)/);
});
