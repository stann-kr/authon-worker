...
export function bookingIssues(data: MockState, b: Booking): BookingIssue[] {
  ...
  const add = (key: string, label: string, action: BookingIssue["action"]) => issues.push({ key, label, action });
  if (!b.owner.trim()) add("owner", "부킹 담당자 지정", "edit");
  if (b.nextAction && (!b.due || b.due <= MOCK_NOW.slice(0, 10)))
    add("followup", b.due ? "오늘까지 후속 업무 처리" : "후속 업무 기한 지정", "edit");
  if (holdExpired(b)) add("hold", "지난 홀드 재협의", "edit");
  if (conflictsFor(b, data.planning.bookings).length) add("conflict", "교체·이동 포함 일정 겹침", "edit");
  ...
  if (b.status === "confirmed") {
    ...
    if (!b.materials.pressUrl || !b.materials.riderUrl) add("materials", "누락된 행사 자료 등록", "edit");
